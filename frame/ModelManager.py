    ...
    def __cross_validation(self, train_dataloader, valid_dataloader):
        ...
        for epoch in range(1, self.config.epoch + 1):
            ...
            if epoch % self.config.interval_valid == 0:
                valid_performance, avg_test_loss, ROC_data, PRC_data, repres_list, label_list = self.__SL_test(valid_dataloader)
    ...
    def __SL_train(self, train_dataloader, test_dataloader):
        ...
        for epoch in range(1, self.config.epoch + 1):
            ...
            if epoch % self.config.interval_test == 0:
                ...
                cur_acc = test_performance[0]  # test_performance: [ACC, Sensitivity, Specificity, AUC, MCC]
                is_best = bool(cur_acc >= best_mcc)
                best_mcc = max(cur_acc, best_mcc)
