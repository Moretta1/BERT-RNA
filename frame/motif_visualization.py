...
def load_tsv(filename, skip_head=True):
    ...
    with open(filename, 'r') as file:
        ...
        for line in file:
            ...
            list = line.split('\t')
            if int(list[1]) == 0:
                neg_sequences.append(list[2])
            else:
                pos_sequences.append(list[2])

    return pos_sequences, neg_sequences

if __name__ == '__main__':

    import seq_visualization
    import os
    ...
    dev_pos, dev_neg = load_tsv('./data/DNA_MS/tsv/6mA/6mA_Xoc BLS256/test.tsv')
