...
class Visualizer():
    ...
    def draw_ROC_PRC_curve(self):
        ...
        plt.fill_between(self.roc_data[0], self.roc_data[1], step='post', alpha=0.2,
                         color='b')
        ...
        plt.plot(self.prc_data[0], self.prc_data[1], color='darkorange',
                 lw=lw, label='PR curve (area = %0.2f)' % self.prc_data[2])  ### 假正率为横坐标，真正率为纵坐标>做曲线
    ...
    def draw_tsne(self):
        ...
        X_tsne = sklearn.manifold.TSNE(n_components=2).fit_transform(data)  # [num_samples, n_components]
    ...
    def draw_umap(self):
        ...
        plt.figure()
        plt.scatter(embedding[:, 0], embedding[:, 1], c=data_index, cmap='viridis', s=5, alpha=0.5)
        plt.gca().set_aspect('equal', 'datalim')
        plt.title('{}-mer UMAP projection '.format(self.config.kmer), fontdict=font)
        if data_label:
            for i in range(len(embedding)):
                plt.annotate(data_label[i], xy=(embedding[:, 0][i], embedding[:, 1][i]),
                             xytext=(embedding[:, 0][i] + 1, embedding[:, 1][i] + 1))
        if data_label is None:
            cbar = plt.colorbar()
            #cbar.set_label(label='digit value', fontdict=font)
            #plt.clim(0 - 0.5, 2 - 0.5)

        plt.savefig('{}/{}.{}'.format(self.IOManager.result_path, "",self.config.save_figure_type))
        plt.show()
